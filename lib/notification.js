...
module.exports = class Notification {
	...
	validate(notification) {
		...
		if(!this.serviceName)
			throw new Error('Service name variable is not defined');

		if(!this.sqsBaseUrl)
			throw new Error('Sqs base url variable is not defined');

		if(!this.notificationAccountId)
			throw new Error('Notification account id variable is not defined');
